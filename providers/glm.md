...
name: trinity-glm
description: |
  Worker agent for GLM-5.2 (via droid exec --auto medium --model glm-5.2 --reasoning-effort high).
...
```bash
RESPONSE=$(droid exec --auto medium --model glm-5.2 --reasoning-effort high "<prompt>" 2>&1)
```
...
```bash
RESPONSE=$(droid exec --auto medium --model glm-5.2 --reasoning-effort high -s "$SESSION_ID" "<prompt>" 2>&1)
```
...
- Always use `droid exec --auto medium --model glm-5.2 --reasoning-effort high` (non-interactive mode)
