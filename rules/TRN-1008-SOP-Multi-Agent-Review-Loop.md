...
**(c) User stop chat input**: If the user types `stop`, `pause`, `hold`, or any explicit halt instruction, the orchestrator MUST: (1) NOT arm the next wake, AND (2) `touch .git/trinity-loop-stopped` to create a stop-marker file. The marker addresses the case where a previously-armed wake is still pending when the stop instruction arrives — that wake will fire later, but the marker is a durable signal it can inspect. **Stop-marker mechanism**: the marker lives at `.git/trinity-loop-stopped` (inside `.git/` so git status doesn't surface it; one path per repo; no gitignore needed). Every §1 idle-retry and §11 loop-restart wake's `prompt=` MUST include a leading guard: `FIRST: if -e .git/trinity-loop-stopped, wake is no-op (user has stopped the loop; do NOT enter phase 1 or arm next wake)`. The guard runs BEFORE the git-branch check (it's the more explicit signal). Resumption: orchestrator removes the marker (`rm .git/trinity-loop-stopped`) on receiving a fresh user-instruction that resumes the loop (e.g., a `/loop` invocation, a new pick directive, or explicit "resume"); OR the operator removes it manually.
