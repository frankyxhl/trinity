...
- **Merge-watch loop**: after the mergeable declaration, arm `ScheduleWakeup(delaySeconds=270, reason="TRN-1008 §10 merge-watch — polling PR #<N> mergedAt", prompt="...")` with a merge-watch prompt that polls `gh pr view <N> --json mergedAt -q .mergedAt` on wake. The 270s cadence matches §8's default polling interval (cache-warm). **On wake**: if `mergedAt` is non-null (merged), run cleanup (`git switch main && git pull --ff-only origin main`) and arm §11. If still pending: arm next merge-watch wake (counter `merge-watch wake N of 12` — same prompt-embedded pattern as §1 idle-retry). Stop conditions: same as §1 idle-with-retry (12 consecutive wakes ≈ 54 min → surface to user; user-stop marker; session termination). Stop-marker check FIRST (per case (c)) before polling. The same two-guard cancellation pattern applies (stop-marker FIRST, then git-branch check — though during merge-watch the orchestrator is typically on a feature branch, so the git-branch guard is NOT used here; only the stop-marker applies).
- **After merge detected + cleanup completes**, proceed to §11 Loop restart.

### 11. Loop restart

**Entry precondition (load-bearing)**: by the end of §10's merge-watch loop (i.e., when Frank's merge is detected and cleanup completes), the orchestrator MUST be on `main` with `origin/main` pulled. §11 fires under that precondition only. If §10's merge-watch exceeded its counter (12 wakes ≈ 54 min) or cleanup failed (merge conflict, network error), surface the failure to the user and do NOT arm the §11 wake — manual intervention required.
...
The same prompt-embedded counter pattern applies to §10 merge-watch wakes (`merge-watch wake N of 12` token); stop condition fires at N==12 (~54 min @ 270s) with surface message: "Merge-watch has been pending for ~54 min on PR #N; pausing. Merge the PR or instruct manually to resume."
