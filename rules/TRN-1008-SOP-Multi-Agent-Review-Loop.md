...
2. The operator's clarification = the most-recent comment AFTER that anchor whose `user.login` is NOT in the agent gh login set (excludes orchestrator self-replies and bot comments such as `iterwheel-blueprint[bot]`, `chatgpt-codex-connector[bot]`).
