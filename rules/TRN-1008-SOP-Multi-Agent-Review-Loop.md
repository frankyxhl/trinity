...
§11 is **idempotent per PR**: if the merge-watch wake fires a second time on a PR whose retro already ran (e.g., due to a stale wake replaying), the orchestrator skips §11 and proceeds directly to §12. Idempotency is enforced via per-PR retro-ran flag (in-session state; persistent re-armament after orchestrator restart re-runs §11 — accepted residual since the operation is read-only except for the optional Step 2 memory write and Step 3 issue file, both gated on user confirmation).
