...
2. TRN-1008 §4 prompt structure AND §8 code-review prompt spec: add one sentence to both prompt specs — *"When scoring compression, exclude the characters of test blocks that pin a contract row your review process demanded (cited by finding ID); audit the mapping, not the growth."* — the same contract-pinning eligibility as the normative TRN-1800 rule; tests demanded for performance, internal structure, or ordinary feature behavior are NOT excluded. (§8's dispatch does not inherit §4's prompt, so the sentence must appear in both.) The §8 prompt edit ALSO carries the declared-vs-actual divergence check: on the first post-implementation re-score, the reviewer computes the deterministic divergence rule (per-block absolute character deltas per component, zero-declared contributing full actual volume, actual `chars merged` measured per the PRP's intent rule — relocations never count toward merged (counted once via chars deleted); the exact counting algorithm is set by the implementing CHG and its tests — aggregated over the CHG, flagged vs 10% of combined declared+actual cited-block volume) and emits a Scope-restraint finding when it trips.
...
- [ ] The exclusion sentence appears in BOTH the §4 plan-review prompt spec and the §8 code-review prompt spec, and the §8 prompt carries the declared-vs-actual divergence check with its deterministic calculation including the implementing CHG's counting algorithm for actual chars merged, which MUST observe the intent rule: no pure relocation ever counts toward merged (relocations count once, via chars deleted)
