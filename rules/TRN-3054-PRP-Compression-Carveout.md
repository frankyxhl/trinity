...
**Rule:** test code that pins a contract row demanded by a review finding is excluded from the compression-ratio calculation — the same contract-pinning eligibility as the normative mechanics below; tests demanded for performance, internal structure, or ordinary feature behavior are NOT excluded — provided the CHG cites the finding ID at the added test block in the full canonical format (e.g., a `# pins P2-glm-F3` comment or a Change-History row mapping test → canonical finding ID). The carve-out applies at every scoring round — plan-review (§4) and code-review iterations (§8/§9) alike — so reviewer-demanded growth stays unpenalized wherever the panel re-scores. Measurement is phase-appropriate: during §4 rounds the CHG is pre-implementation, so compression — and the exclusion — is computed on the CHG's declared character budget: the plan's surface enumeration states per-block added/deleted/merged character estimates for every planned block (merged defaults to 0 for a block unless the plan explicitly declares a consolidation), and cited test blocks carry the finding-ID mapping. §4 reviewers compute from those declared numbers (deterministic inputs — same plan, same ratio and exclusion; the ratio-to-score mapping remains reviewer judgment per the D24 ruling). The first post-implementation re-score (§8) recomputes on actual characters, with the shipped test blocks replacing the plan entries in the same mapping; divergence is computed deterministically: per cited block, sum the absolute character deltas per component (|actual − declared| for added, deleted, and merged separately; a component declared with the zero default contributes its full actual volume to the delta). Actual `chars merged` is measured by a canonical computation independent of diff presentation: `git diff --unified=0` (minimal hunks, no context, no inter-hunk fusion — the same edits always yield the same changed regions). A changed region is one delete-run plus its paired add-run between two consecutive unchanged anchor lines. `chars merged` = the characters of distinct lines present in BOTH the delete-run and the paired add-run of the SAME changed region — content rewritten in place by a consolidation. Verbatim relocations, near or far, produce a delete-run and an add-run separated by unchanged anchors (different changed regions) and contribute 0 to merged; their text counts once, in `chars deleted`. No reviewer judgment, no dependence on --unified/--inter-hunk-context rendering; aggregate those sums over the CHG, and flag a Scope-restraint finding when the aggregate exceeds 10% of the total (declared + actual) character volume of all cited blocks combined.
...
2. TRN-1008 §4 prompt structure AND §8 code-review prompt spec: add one sentence to both prompt specs — *"When scoring compression, exclude the characters of test blocks that pin a contract row your review process demanded (cited by finding ID); audit the mapping, not the growth."* — the same contract-pinning eligibility as the normative TRN-1800 rule; tests demanded for performance, internal structure, or ordinary feature behavior are NOT excluded. (§8's dispatch does not inherit §4's prompt, so the sentence must appear in both.) The §8 prompt edit ALSO carries the declared-vs-actual divergence check: on the first post-implementation re-score, the reviewer computes the deterministic divergence rule (per-block absolute character deltas per component, zero-declared contributing full actual volume, actual `chars merged` measured by the canonical `git diff --unified=0` changed-region computation — a line counts only when its deleted and added occurrences share one minimal changed region; relocations (different regions) contribute 0 to merged, counted once via chars deleted — aggregated over the CHG, flagged vs 10% of combined declared+actual cited-block volume) and emits a Scope-restraint finding when it trips.
...
- [ ] The exclusion sentence appears in BOTH the §4 plan-review prompt spec and the §8 code-review prompt spec, and the §8 prompt carries the declared-vs-actual divergence check with its deterministic calculation including the canonical git diff --unified=0 changed-region measurement of actual chars merged (relocations excluded from merged, counted once via chars deleted)
