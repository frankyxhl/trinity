...
| 2026-07-04 | PR #276 R1 (codex bot P3): `--project-dir` default was eagerly `os.getcwd()` at parser build, crashing `--version` under a deleted cwd (Preserved-contract regression vs the manual parser). Fix: `default=None`, resolved lazily at list-dispatch (`args.project_dir or os.getcwd()`). Regression test `test_chg3047_version_with_deleted_cwd` pins it (verified RED against ce9fbf0, GREEN post-fix). Code-review tier passed pre-R1: glm 9.5 / deepseek 9.50. | trinity-glm + trinity-deepseek |
