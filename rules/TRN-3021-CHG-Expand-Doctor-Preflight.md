...
| 2026-05-08 | PR #68 CI bug-fix: 12 tests failed in CI because `wrapper_auth_check(provider)` matched by NAME inside `provider_health()`, which is shared between `cmd_doctor` AND `cmd_review --check-providers` / review preflight. Test fixtures with fake `tmp_path/bin/<provider>` scripts triggered the canonical `~/.secrets/<provider>_api_key` lookup → auth-missing leaked into review-flow tests. **Architectural fix**: removed auth-check from `provider_health` body; `cmd_doctor` now runs `wrapper_auth_check` post-hoc, gated on `_is_canonical_wrapper(executable, provider)` — matches `/skills/trinity/bin/<provider>` substring or `/providers/bin/<provider>` suffix; tmp_path fixtures don't match. cmd_review preflight is unaffected (matches pre-TRN-3021 behavior). Test `test_a4c_wrong_mode_routes_to_issues_for_required` removed (auth check now lives in cmd_doctor only; wrapper_auth_check unit tests A4/A4b/A4c-mode-600/400/644 still cover helper behavior). 251 tests pass, lint clean. Dispatched as trinity-glm coding worker per new operating model. | Claude Opus 4.7 |
