...
| 2026-05-08 | PR #60 round 4 from Codex GitHub App bot — caught a **fourth sibling-code bug**: adding `status` as a built-in subcommand without also reserving it in `PRESET_ALIAS_RESERVED_WORDS = {"init-config", "doctor", "review", "help"}` (scripts/codex.py:30). Without the update, configs can define `preset_aliases: {"status": "review"}` and `trinity doctor/review --preset status` accepts the alias instead of failing — leaving `status` ambiguous between built-in subcommand and user alias. Fix: add `"status"` to the reserved-words set. Added regression test `test_codex_review_alias_collision_status_subcommand` asserting `trinity: preset alias 'status' collides with subcommand` is raised. **Four consecutive bot findings of the same class — the structural gap is no longer hypothetical, it is empirically validated.** Applying the methodology fix mid-flight per operator instruction: code-review agent prompts MUST require (1) trace caller flows of any function the diff consumes data from; (2) read writer schemas of any data the diff parses; (3) compare new subcommand/feature to all sibling registration sites (subcommand dispatch table, reserved-word sets, config validators, help text registries). This will land in the next CHG that introduces a new code surface (TRN-2026 / 2027 / 3021 / 3020). | Claude Opus 4.7 |
| 2026-05-08 | PR #60 round 5 from Codex GitHub App bot — caught a **fifth caller-flow bug** in the very helper added for R1: `cmd_review`'s `ReviewOrchestrationError` handler (scripts/codex.py:1385) writes `incomplete.json` with `status="failed"` (and the bare `Exception` handler at :1396 likewise), but `_print_incomplete_only_summary` hard-coded both labels as "interrupted" (`Latest review: ... (interrupted at ...)` and `Status: interrupted (<status>)`). Result: a real orchestration failure was misrendered as "Status: interrupted (failed)" — conflating user-cancelled and orchestration-failed states in exactly the no-metadata path added for the failure case. Fix: use the stored `status` field as the top-level label directly (`(<status> at <when>)` and `Status: <status>`). T12 + T12b updated to assert the new labels and to *negatively* assert that "Status: interrupted" / "interrupted at" do NOT appear when status is "failed". This is the **fifth consecutive caller-flow / sibling-code finding** — confirms beyond doubt the structural review-methodology gap and validates the mid-flight 3-step methodology fix already logged. Sequence: R1 caller-flow (cmd_review interrupt schema) → R2 writer schema (cleanup.result) → R3 sibling pattern (resolve_health_root) → R4 sibling registry (PRESET_ALIAS_RESERVED_WORDS) → R5 writer schema again (incomplete.status field's actual values). | Claude Opus 4.7 |
| 2026-05-08 | PR #60 round 6 from Codex GitHub App bot — caught the **sixth bug**, the exact same misrendering as R5 but in the sibling code branch (`_print_review_summary` metadata-PRESENT path, scripts/codex.py:1609). When an exception happens AFTER `metadata.json` is written (e.g. `write_synthesis()` raises after lines 1365-1369), `cmd_review` writes `incomplete.json` with `status="failed"` alongside the metadata — and the metadata-present overall-label code still hard-coded `f"interrupted ({incomplete_status})"`. Same fix: use `overall = incomplete_status` directly. Added T14 regression test for metadata-present + incomplete.status='failed' (T12b deletes metadata.json so cannot cover this branch). **R5 and R6 are literally the same bug class in adjacent code paths — I fixed one and missed its mirror.** Highlights another methodology refinement: when fixing a misuse of an enum/status value in one helper, grep for the same hard-code in sibling/parallel helpers before declaring the fix complete. Adding to the queued 3-step methodology rule: (4) when fixing a value-handling bug in helper X, grep for the same value/pattern in all helpers that consume the same data source. | Claude Opus 4.7 |
| 2026-05-08 | PR #60 round 7 from Codex GitHub App bot — caught a **seventh bug**: `cmd_status` used `sorted(reviews, reverse=True)` over full directory names, claiming in a comment that "fixed-width %Y%m%d-%H%M%S prefix → lex order == chronological order." But `make_review_dir()` (scripts/codex.py:914) only stamps to seconds and appends `<slug>[-<index>]`. Two reviews created in the same second with different slugs (`-z` vs `-a`) or numeric collision suffixes (`-10` vs `-2`) have lex order != creation order, so `trinity status` could pick the older same-second review as "latest". Fix: sort by `(d.name[:15], d.stat().st_mtime)` — fixed-width prefix gives chronological correctness across distinct seconds; mtime tiebreaks same-second cases. Added T15 regression test: creates `-z` and `-a` with identical timestamp prefix, force-sets mtimes so `-a` is newer, asserts `-a` (not `-z`) is selected as "Latest review". **R7 is a different bug class — not caller-flow / sibling-code, but an unverified assumption stated in a code comment.** Adding methodology rule (5): code comments asserting an invariant (e.g., "fixed-width prefix → lex == chrono") must be verified against the writer that produces the data. The R3-style "compare to writer" rule was scoped to data schemas; this case extends it to *invariants stated as comments*. The queued methodology fix is now 5 steps. | Claude Opus 4.7 |
