...
| `glm` | `providers/registry.json` → `providers.glm.cli` + `.agents/trinity.codex.json` + `providers/glm.delta.md` | Claude Code install: `droid exec --auto medium --model glm-5.2 --reasoning-effort high`; Codex review: `droid exec --model glm-5.2 --reasoning-effort high` |
