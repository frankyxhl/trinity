...
check "CodeQL init uses pinned SHA with version comment" grep -qE 'uses: github/codeql-action/init@[0-9a-f]{40} # v[0-9]+' "$WORKFLOW"
check "CodeQL analyze uses pinned SHA with version comment" grep -qE 'uses: github/codeql-action/analyze@[0-9a-f]{40} # v[0-9]+' "$WORKFLOW"
