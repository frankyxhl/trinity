...
check "setup-python uses pinned SHA with version comment" grep -qE 'uses: actions/setup-python@[0-9a-f]{40} # v[0-9]+' "$WORKFLOW"
check "workflow uses Python 3.11" grep -qF "python-version: '3.11'" "$WORKFLOW"
check "setup-uv uses pinned SHA with version comment" grep -qE 'uses: astral-sh/setup-uv@[0-9a-f]{40} # v[0-9]+' "$WORKFLOW"
