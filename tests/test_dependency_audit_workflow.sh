...
check "setup-python uses pinned major" grep -qF 'uses: actions/setup-python@v6' "$WORKFLOW"
check "workflow uses Python 3.11" grep -qF "python-version: '3.11'" "$WORKFLOW"
check "setup-uv uses pinned major" grep -qF 'uses: astral-sh/setup-uv@v6' "$WORKFLOW"
