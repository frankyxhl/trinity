...
def test_missing_both_files_returns_empty(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, str(SCRIPT), "merge", "--global-config", str(global_config), "--project-dir", str(project_dir)],
        capture_output=True,
        text=True,
    )
...
def test_global_only_returns_global_no_sessions(tmp_path):
    ...
    project_dir = tmp_path / "project"
    rc, out, err = run(["merge", "--global-config", str(global_config), "--project-dir", str(project_dir)])
    assert rc == 0
...
def test_providers_project_key_wins_on_conflict(tmp_path):
    global_config = tmp_path / "global" / "trinity.json"
    write_config(global_config, {
        "providers": {"glm": {"cli": "droid-global"}, "codex": {"cli": "codex-global"}}
    })
    project_dir = tmp_path / "project"
    project_trinity = project_dir / ".claude" / "trinity.json"
    write_config(project_trinity, {
        "providers": {"glm": {"cli": "droid-project"}}
    })
    rc, out, err = run(["merge", "--global-config", str(global_config), "--project-dir", str(project_dir)])
    assert rc == 0
    result = json.loads(out)
...
def test_invalid_json_in_global_exits_1(tmp_path):
    ...
    rc, out, err = run(["merge", "--global-config", str(global_config), "--project-dir", str(project_dir)])
    assert rc == 1
    assert "trinity-scripts" in err or "invalid JSON" in err or str(global_config) in err
...
def test_invalid_json_in_project_exits_1(tmp_path):
    ...
    rc, out, err = run(["merge", "--global-config", str(global_config), "--project-dir", str(project_dir)])
    assert rc == 1
    assert "trinity-scripts" in err or "invalid JSON" in err or str(project_trinity) in err
